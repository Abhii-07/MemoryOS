...
Press Enter to accept defaults. These fill `gemini-3.5-flash`, `gemini-3.5-flash`, etc. everywhere
...
- [x] `DONE.html` section 1 (cognitive job) filled — G0
- [x] `.genesis/` scaffolded — G1
- [x] `context-graph.json` has nodes, edges, ≥2 invariants — G2
- [x] `wiki/index.md` points at relevant swe-kit concepts — G3
- [x] `DONE.html` section 2 (definition of done) filled from phase gates — G4
- [x] `PLAN.md` has ≥1 milestone with a real demo command — G5
- [x] `KICKOFF.md` placeholders replaced — G6
