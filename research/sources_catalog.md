...
| MIRIX | Memory research framework/benchmark (agent memory) | Evaluation approach for memory retrieval quality | week-3 |
...
| [P:5] | Retrieval-Augmented Generation (Lewis et al., 2020) | RAG pattern: retrieval bounds context, grounds answers |
...
| [P:8] | Generative Agents (Park et al., 2023) | Observation → reflection → consolidation lifecycle; error reinforcement |
...
| (pending) | LoCoMo (long-conversation memory benchmark) | D4 readiness eval — verify in R2 |
| (pending) | PIIBench (PII leak benchmark) | OOD PII detection instability (0.96→0.18) — verify in R1 |
...
|---|---|---|
| OWASP LLM AI Security (ASI06) 2026 taxonomy | Threat taxonomy | Required threat-model vocabulary for D4 `threat_model.md` |
...
2. Before D4: run research pass **R1** (verify each row's claims + citations), then **R2** (fill the
   `(pending)` benchmark rows), then **R3** (product positioning against Supermemory/Mem0/Zep).
3. Update this file's *verification status* during those passes; never cite a claim marked unverified.